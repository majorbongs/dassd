...
function addTalker(plySource)
	radioTalkers[plySource] = true
	refreshTalkingPlayers()
end

function removeTalker(plySource)
	radioTalkers[plySource] = false
	refreshTalkingPlayers()
end

function clearTalkers()
	radioTalkers = {}
	refreshTalkingPlayers()
end

function refreshTalkingPlayers()
	local talkers = {}
	for k, v in pairs(radioTalkers) do 
		if v then 
...
RegisterCommand('+radiotalk', function()
	...
	if IsControlPressed(2, 24) or IsControlPressed(2, 25) then return end -- can't use radio while shooting
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			local enableRadioAnim = GetConvarInt('voice_enableRadioAnim', 0) == 1
			if enableRadioAnim and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) then
				if not disableRadioAnim then
					RequestAnimDict('radiotalk2@law')
					while not HasAnimDictLoaded('radiotalk2@law') do
						Citizen.Wait(10)
					end
						TaskPlayAnim(PlayerPedId(), "radiotalk2@law", "radiotalk2_clip", 8.0, 2.0, -1, 50, 2.0, 0, 0, 0)
				end
			end
			Citizen.CreateThread(function()
				...
				while radioPressed do
					...
					if (enableRadioAnim and IsPedShooting(PlayerPedId())) then
						StopAnimTask(PlayerPedId(), "radiotalk2@law", "radiotalk2_clip", -4.0)
					end
