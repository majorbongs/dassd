...
---@param clickType integer indicating the sound: 0: off, 1: on, 2: static sound
function playMicClicks(clickType)
	local soundInfo = {
		...
		[2] = {
			soundName = "static_off",
			volumeMult = 1.0
		}
	}

	if micClicks ~= 'true' and clickType ~= 2 then 
		return logger.verbose("Not playing mic clicks because client has them disabled") 
	end
	
	sendUIMessage({
		sound = (soundInfo[clickType].soundName),
		volume = (soundInfo[clickType].volumeMult * volumes["radio"])
	})
end

--- toggles the targeted player muted
---@param source number the player to mute
function toggleMute(source, toggle)
	if toggle then
