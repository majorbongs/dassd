...
public class UIMenuProgressItem : UIMenuItem
{
	...
	{
		((Rectangle)_bar).Size = new SizeF((float)Index * (_max / (float)Items), ((Rectangle)_bar).Size.Height);
	}
	...
	public override void Position(int y)
	{
		...
		((Rectangle)_background).Size = new SizeF(415f + (float)base.Parent.WidthOffset, 14f);
		((Rectangle)_background).Position = new PointF(8f + base.Offset.X, 170f + (float)y + base.Offset.Y);
		((Rectangle)_bar).Position = new PointF(11.75f + base.Offset.X, 172.5f + (float)y + base.Offset.Y);
	}

	public void CalculateProgress(float CursorX)
	{
		float num = CursorX - ((Rectangle)_bar).Position.X;
		Index = (int)Math.Round((float)Items * ((num >= 0f && num <= _max) ? num : ((num < 0f) ? 0f : _max)) / _max);
	}

	public async void Functions()
	{
		if (ScreenTools.IsMouseInBounds(new PointF(((Rectangle)_bar).Position.X, ((Rectangle)_bar).Position.Y - 7.5f), new SizeF(_max, ((Rectangle)_bar).Size.Height + 19f)) && API.IsDisabledControlPressed(0, 24) && !Pressed)
		{
			Pressed = true;
			Audio.Id = API.GetSoundId();
			API.PlaySoundFrontend(Audio.Id, Audio.Slider, Audio.Library, true);
			while (API.IsDisabledControlPressed(0, 24) && ScreenTools.IsMouseInBounds(new PointF(((Rectangle)_bar).Position.X, ((Rectangle)_bar).Position.Y - 7.5f), new SizeF(_max, ((Rectangle)_bar).Size.Height + 19f)))
			{
