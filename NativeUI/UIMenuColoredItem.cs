...
public class UIMenuColoredItem : UIMenuItem
{
	...
	{
		((Rectangle)_rectangle).Size = new SizeF(431 + base.Parent.WidthOffset, 38f);
		_selectedSprite.Size = new SizeF(431 + base.Parent.WidthOffset, 38f);
		if (Hovered && !Selected)
		{
			((Rectangle)_rectangle).Color = Color.FromArgb(20, 255, 255, 255);
		...
		((Text)_text).Color = ((!Enabled) ? Color.FromArgb(163, 159, 148) : (Selected ? HighlightedTextColor : TextColor));
		if (LeftBadge != BadgeStyle.None)
		{
			((Text)_text).Position = new PointF(35f + base.Offset.X, ((Text)_text).Position.Y);
		...
		{
			((Text)_text).Position = new PointF(8f + base.Offset.X, ((Text)_text).Position.Y);
		}
		...
		{
			((Text)_labelText).Position = new PointF(420f + base.Offset.X + (float)base.Parent.WidthOffset, ((Text)_labelText).Position.Y);
			((Text)_labelText).Caption = RightLabel;
			UIResText labelText = _labelText;
			Color color = (((Text)_text).Color = ((!Enabled) ? Color.FromArgb(163, 159, 148) : (Selected ? HighlightedTextColor : TextColor)));
