...
public class UIMenuSliderItem : UIMenuItem
{
	...
	{
		((Rectangle)_rectangleBackground).Position = new PointF(250f + base.Offset.X, (float)(y + 158) + base.Offset.Y);
		((Rectangle)_rectangleSlider).Position = new PointF(250f + base.Offset.X, (float)(y + 158) + base.Offset.Y);
		((Rectangle)_rectangleDivider).Position = new PointF(323f + base.Offset.X, (float)(y + 153) + base.Offset.Y);
	...
	public override async Task Draw()
	{
		...
		float num = 176f + base.Offset.X + ((Rectangle)_rectangleBackground).Size.Width - ((Rectangle)_rectangleSlider).Size.Width;
		((Rectangle)_rectangleSlider).Position = new PointF((int)(num + (float)_value / (float)_max * 73f), ((Rectangle)_rectangleSlider).Position.Y);
		if (Selected)
		{
			_arrowLeft.Draw();
			_arrowRight.Draw();
