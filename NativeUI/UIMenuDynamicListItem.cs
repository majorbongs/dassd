...
public class UIMenuDynamicListItem : UIMenuItem, IListItem
{
	...
	{
		base.Draw();
		string currentListItem = CurrentListItem;
		float textWidth = ScreenTools.GetTextWidth(currentListItem, ((Text)_itemText).Font, ((Text)_itemText).Scale);
		((Text)_itemText).Color = ((!Enabled) ? Color.FromArgb(163, 159, 148) : (Selected ? Colors.Black : Colors.WhiteSmoke));
		((Text)_itemText).Caption = currentListItem;
