...
XNL_MaxPlayerLevel = 7999  			-- Sets the maximum level (Maximum which can be used is 7999!)
...
RegisterNetEvent("Exp_XNL_SetHidden")
AddEventHandler("Exp_XNL_SetHidden", function(hidden)
	XNL_Hidden = hidden
end)
...
function XNL_GetXPFloorForLevel(intLevelNr)
	if is_int(intLevelNr) then
		...
		if intLevelNr > MAX_LEVEL then
			BaseXP = PLAYER_LEVELS[MAX_LEVEL - 1]	-- Get the 'base XP' from the last known original rockstar level (100) in my table
			ExtraAddPerLevel = 25		-- This is the amount which rockstar ADDS on the amount required for the next level PER next level
			MainAddPerLevel = BaseXP - 50 	-- This is the amount of XP required for level 100 ***MINUS 50 so my "formula" will work correctly!***
			...
			BaseLevel = intLevelNr - MAX_LEVEL -- Here we make sure that the first 100 levels are 'excluded' from the loop since we already have a base offset.
			CurXPNeeded = 0				 -- This variable will remember the total XP for the level (and adds to it during the loop below)
			for i = 1, BaseLevel ,1 
			do 
				MainAddPerLevel = MainAddPerLevel + ExtraAddPerLevel		-- Adding 50 extra XP per new level we pass in the loop
		...
		return PLAYER_LEVELS[intLevelNr - 1]
	else
		print("=====================================================================================================")
...
function XNL_GetXPCeilingForLevel(intLevelNr)
	if is_int(intLevelNr) then
		...
		if intLevelNr > MAX_LEVEL - 1 then
			BaseXP = PLAYER_LEVELS[MAX_LEVEL - 1]
			ExtraAddPerLevel = 25		-- This is the amount which rockstar ADDS on the amount required for the next level PER next level
			MainAddPerLevel = PLAYER_LEVELS[MAX_LEVEL - 1] - 50	-- This is the amount of XP required for level 100 ***MINUS 50 so my "formula" will work correctly!***
			...
			BaseLevel = intLevelNr - (MAX_LEVEL - 1)
			CurXPNeeded = 0
			for i = 1, BaseLevel ,1 
			do 
		...
		-- required from the table I've put in this script which uses the original rockstar XP levels
		return PLAYER_LEVELS[intLevelNr]
	else
		print("=====================================================================================================")
...
function XNL_GetLevelFromXP(intXPAmount)
	...
	if is_int(intXPAmount) then
		...
		if SearchingFor < PLAYER_LEVELS[MAX_LEVEL - 1] then			-- Check if the XP amount is smaller than the last item (level 100) in my 'Rockstar XP Requirements level list'
			local CurLevelFound = -1
		
			local CurrentLevelScan = 0
			for k,v in pairs(PLAYER_LEVELS)do			-- And if it's bellow the 'maximum known rockstar XP requirement' scan the table here
			...
			CurXPNeeded = 0
			local CurLevelFound = -1
			for i = 1, XNL_MaxPlayerLevel - (MAX_LEVEL - 1) ,1 			-- The - 99 in this loop ensures that the for loop result excludes the first 99 levels which are 
			...
			return CurLevelFound + MAX_LEVEL - 1
		end
	else
		print("=====================================================================================================")
