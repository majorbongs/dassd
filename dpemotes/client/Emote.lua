...
local isCancelingScenario = false

Citizen.CreateThread(function()
  while true do

    if Config.MenuKeybindEnabled then
      if IsControlJustPressed(0, Config.MenuKeybind) and IsInputDisabled(2) then
        OpenEmoteMenu()
      end
    end
...
AddEventHandler('dpemotes:cancelEmoteImmediately', function ()
  DestroyAllProps()
  ClearPedTasksImmediately(GetPlayerPed(-1))
  ResetPedMovementClipset(PlayerPedId())
...
function EmoteCancel()
  ...
  if IsInAnimation and (ChosenDict == "Scenario" or ChosenDict == "MaleScenario") then
  ...
  TriggerServerEvent("gtacnr:canceledEmote")
end

function CancelScenario()
  if isCancelingScenario then
    return
  end

  isCancelingScenario = true;

  Citizen.CreateThread(function()
    local playerPed = PlayerPedId();
    while IsPedRagdoll(playerPed) or IsPedGettingUp(playerPed) do
      Citizen.Wait(100)
    end
    ...
    isCancelingScenario = false
  end)
end

--Ragdoll detection for automatic scenario reset
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(500)
    ...
    ---@type number
    local weaponHash;
    isUsingWeapon, weaponHash = GetCurrentPedWeapon(playerPed);
    if (IsInAnimation and (((ChosenDict == "Scenario" or ChosenDict == "MaleScenario") and IsPedRagdoll(playerPed)) or (isUsingWeapon and weaponHash ~= 966099553) or IsPedInAnyVehicle(PlayerPedId(), true))) then
      EmoteCancel();
    end
  end
end)
...
AddPropToPlayer = function(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
  ...
  PlayerHasProp = true
  TriggerServerEvent("gtacnr:ac:registerEntity", NetworkGetNetworkIdFromEntity(prop))
end
...
function OnEmotePlay(EmoteName)
  ...
  if IsPedInAnyVehicle(playerPedId, true) 
      ...
      or IsPedInParachuteFreeFall(playerPedId) 
      or IsPedSwimming(playerPedId)
      or IsPedSwimmingUnderWater(playerPedId)
      or IsPedRagdoll(playerPedId)
      or IsPedGettingUp(playerPedId) then 
    return
  end
  ...
  TriggerServerEvent("gtacnr:usedEmote", ChosenDict, ChosenAnimation, ename)
  ...
  if ChosenDict == "MaleScenario" or "Scenario" then
    ...
    if ChosenDict == "MaleScenario" then
      if PlayerGender == "male" then
        ...
        EmoteChatMessage("This emote is not available for the female character model.")
      end return
