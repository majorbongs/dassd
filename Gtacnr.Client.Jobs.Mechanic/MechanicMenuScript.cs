...
public class MechanicMenuScript : Script
{
	...
	protected override async void OnStarted()
	{
		...
		Menu = new Menu(LocalizationController.S(Entries.Businesses.MENU_MECHANIC_TITLE), LocalizationController.S(Entries.Main.MENU_CHOOSE_OPTION));
	...
	{
		if (e.PreviousJobEnum != JobsEnum.Mechanic && e.CurrentJobEnum == JobsEnum.Mechanic)
		{
			RefreshJobMenu();
		}
	}
	...
	private async void RefreshJobMenu()
	{
		...
		MechanicMenuScript mechanicMenuScript = this;
		Menu callsMenu = DispatchScript.MechanicDispatch.CallsMenu;
		Dictionary<string, MenuItem> dictionary = menuItems;
		MenuItem obj = new MenuItem(LocalizationController.S(Entries.Businesses.JOBMENU_CALLS), LocalizationController.S(Entries.Businesses.MENU_MECHANIC_CALLS_DESCRIPTION))
		{
			LeftIcon = MenuItem.Icon.GTACNR_JOB_CALLS
		};
		MenuItem menuItem = obj;
		dictionary["calls"] = obj;
		mechanicMenuScript.AddSubmenuItem(callsMenu, menuItem);
		MechanicMenuScript mechanicMenuScript2 = this;
		Menu subMenu = (subMenus["sales"] = new Menu(LocalizationController.S(Entries.Businesses.MENU_MECHANIC_TITLE), LocalizationController.S(Entries.Businesses.JOBMENU_RECENT_SALES_SUB)));
		...
		mechanicMenuScript2.AddSubmenuItem(subMenu, menuItem);
