...
public class PrivateMedicDispatch : BaseCallDispatch<CallInfo>
{
	public PrivateMedicDispatch()
		: base(LocalizationController.S(Entries.Businesses.MENU_PRIVATE_MEDIC_TITLE), LocalizationController.S(Entries.Businesses.JOBMENU_CALLS))
	{
	}
	...
	public override async void OnDispatch(int playerId, string _)
	{
		...
		if (playerState != null)
		{
			...
			AddCall(callInfo);
			await InteractiveNotificationsScript.Show(playerState.ColorNameAndId + " needs a doctor in ~y~" + locationName, InteractiveNotificationType.HelpText, OnAccepted, TimeSpan.FromSeconds(10.0), 0u, "Respond", LocalizationController.S(Entries.Businesses.STP_PRESS, "Respond"));
		}
		bool OnAccepted()
		{
