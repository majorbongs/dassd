...
public class PrivateMedicMenuScript : Script
{
	...
	protected override async void OnStarted()
	{
		...
		Menu = new Menu(LocalizationController.S(Entries.Businesses.MENU_PRIVATE_MEDIC_TITLE), LocalizationController.S(Entries.Main.MENU_CHOOSE_OPTION));
	...
	{
		if (e.PreviousJobEnum != JobsEnum.PrivateMedic && e.CurrentJobEnum == JobsEnum.PrivateMedic)
		{
			RefreshJobMenu();
		}
	}
	...
	private async void RefreshJobMenu()
	{
		...
		menuItems["services"] = new MenuItem(LocalizationController.S(Entries.Businesses.JOBMENU_SERVICES), LocalizationController.S(Entries.Businesses.JOBMENU_SERVICES_DESCR))
		{
			LeftIcon = MenuItem.Icon.GTACNR_SERVICES
		};
		Menu.AddMenuItem(menuItems["services"]);
		MenuController.AddSubmenu(Menu, ServicesMenuScript.Menu);
		MenuController.BindMenuItem(Menu, ServicesMenuScript.Menu, menuItems["services"]);
		PrivateMedicMenuScript privateMedicMenuScript = this;
		Menu callsMenu = DispatchScript.PrivateMedicDispatch.CallsMenu;
		Dictionary<string, MenuItem> dictionary = menuItems;
		MenuItem obj = new MenuItem(LocalizationController.S(Entries.Businesses.JOBMENU_CALLS), LocalizationController.S(Entries.Businesses.MENU_PRIVATE_MEDIC_CALLS_DESCRIPTION))
		{
			LeftIcon = MenuItem.Icon.GTACNR_JOB_CALLS
		};
		MenuItem menuItem = obj;
		dictionary["calls"] = obj;
		privateMedicMenuScript.AddSubmenuItem(callsMenu, menuItem);
		PrivateMedicMenuScript privateMedicMenuScript2 = this;
		Menu subMenu = (subMenus["sales"] = new Menu(LocalizationController.S(Entries.Businesses.MENU_PRIVATE_MEDIC_TITLE), LocalizationController.S(Entries.Businesses.JOBMENU_RECENT_SALES_SUB)));
		...
		privateMedicMenuScript2.AddSubmenuItem(subMenu, menuItem);
