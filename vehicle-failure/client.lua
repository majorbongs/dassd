...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			healthEngineDeltaScaled = healthEngineDelta * Config.damageFactorEngine

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * Config.damageFactorBody
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * Config.damageFactorPetrolTank
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > healthEngineCurrent then
						healthEngineCombinedDelta = healthEngineCurrent
					end
				...
				fWeaponDamageMult = GetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult')
				fBrakeForce = GetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fBrakeForce')
				local newFDeformationDamageMult = fDeformationDamageMult * Config.deformationMult
				...
				if Config.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult', fWeaponDamageMult * Config.weaponsDamageMultiplier) end -- Set weaponsDamageMultiplier

				--Get the CollisionDamageMultiplier
				fCollisionDamageMult = GetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fCollisionDamageMult')
				--
				local newFCollisionDamageMultiplier = fCollisionDamageMult * Config.collisionDamageMult
				...
				--
				local newFEngineDamageMult = fEngineDamageMult * Config.engineDamageMult
			...
			if pedInSameVehicleLast == true then
				...
				if Config.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult', fWeaponDamageMult) end	-- Since we are out of the vehicle, we should no longer compensate for bodyDamageFactor
...
exports("SetVehicleEngineHealth", function(_vehicle, _health)
	if vehicle == _vehicle then
		healthEngineCurrent = _health
		healthEngineLast = _health
	end
